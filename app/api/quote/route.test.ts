...
// Own IP counter/range so it can't collide with app/api/contact/route.test.ts's
// counter even if both files' tests interleave within the same worker.
let ipCounter = 0
function freshIp() {
  ipCounter += 1
...
function makeRequest(
  ...
  return new NextRequest("https://dnaaircargo.com.br/api/quote", {
    method: "POST",
    headers,
...
describe("POST /api/quote", () => {
  ...
  it("returns 400 on an invalid payload (bad transportType) and never sends email", async () => {
    const request = makeRequest({ ...VALID_BODY, transportType: "trem" })
  ...
  it("returns 200 { success: true } on a valid payload, translating transportType to a PT-BR label", async () => {
    sendNotificationEmailMock.mockResolvedValue({ success: true })
    ...
    expect(sendNotificationEmailMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        html: expect.stringContaining("Aéreo"),
      })
    )
  })
  ...
  it("escapes HTML in user-supplied fields (XSS in the notification email)", async () => {
    ...
    const request = makeRequest({
      ...VALID_BODY,
      cargoType: '<img src=x onerror="alert(1)">',
    })
  ...
  it("returns 502 with a generic retry message when email send_failed", async () => {
    ...
    expect(response.status).toBe(502)
    const json = await response.json()
    expect(json).toEqual({ error: "Não foi possível enviar sua solicitação. Tente novamente." })
  ...
  it("returns 400 without throwing when the request body isn't valid JSON", async () => {
    const request = new NextRequest("https://dnaaircargo.com.br/api/quote", {
      method: "POST",
      headers: {
