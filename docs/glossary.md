...
- **Formulário de Contato**: segundo formulário do site (nome, e-mail, telefone opcional, assunto opcional, mensagem), na página Contato, para dúvidas/solicitações que não são pedidos de orçamento estruturados. Schema próprio, reusando o DAL de envio de e-mail do orçamento. Ver [0013](adr/0013-contact-form-shared-email-dal.md).

## Arquitetura

- **DAL (Data Access Layer)**: camada em `lib/data/` que concentra regra de negócio e chamadas a serviços externos (ex: Resend), mantendo os Route Handlers (`app/api/*/route.ts`) finos — só validam e delegam. Ver [0005](adr/0005-component-conventions.md) e [0015](adr/0015-route-handlers-as-bff.md).
...
- **Dry-run (Resend)**: modo de teste (`E2E_TEST_MODE=true`) em que o DAL de e-mail loga o payload em vez de chamar a API do Resend, usado nos testes E2E para não disparar e-mails reais. Ver [0011](adr/0011-testing-strategy.md).
- **`sendNotificationEmail`**: função única do DAL (`lib/data/email.ts`) que concentra a chamada à API do Resend (client, dry-run, tratamento de erro), reusada pelos Route Handlers de orçamento e de contato. Ver [0013](adr/0013-contact-form-shared-email-dal.md).
- **`isTrustedOrigin`**: checagem manual de `Origin` vs `Host` em `lib/security.ts`, reimplementando a proteção que Server Actions davam de graça — necessária porque Route Handlers não têm isso por padrão. Ver [0015](adr/0015-route-handlers-as-bff.md).
